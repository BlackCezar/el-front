...
export default function BRSStudent({ user }) {
    const [getPoints, { data: points, isSuccess, isLoading }] =
    ...
    const renderSSO = React.useCallback(
        (student, half) => {
            if (sso && sso2) {
                if (half === 2) {
                    const ssoStudent2 =
                        sso2 && sso2.length
                            ? sso2.find((i) => i._id === student._id)
                            : 0
    ...
    const renderTable = React.useCallback(
        (student) => {
            const pointsList =
                points && isSuccess
                    ? points.filter((p) => p.student === student._id)
                    : []

            return (
                <Tr>
                    <Td borderWidth={1}>{renderSSO(student, 1)}</Td>
                    <Td borderWidth={1}>
                    ...
                    <Td borderWidth={1}>
                        {pointsList
                            ...
                            parseFloat(renderSSO(student, 1))}
                    </Td>
                    <Td borderWidth={1}>{renderSSO(student, 2)}</Td>
                    <Td borderWidth={1}>
                    ...
                    <Td borderWidth={1}>
                        {pointsList.reduce(
                            ...
                            parseFloat(renderSSO(student, 2)) +
                            parseFloat(renderSSO(student, 1))}
                    </Td>
                </Tr>
            )
                    ...
                    {isLoading ? (
                        ...
                        <TableContainer>
                            <Table borderWidth={1} variant="simple">
                                ...
                                <Tbody>{user && renderTable(user)}</Tbody>
                            </Table>
                        </TableContainer>
                    )}
                </Box>

...
import { Box, Divider, Heading, Table, Tr, Spinner, Th, Thead, TableContainer, Center, HStack } from '@chakra-ui/react'
...
export default function DashboardStudent() {
    ...
    const renderBox = useCallback((d, i) => {
        const newD = new Date(d.getTime())
        newD.setDate(newD.getDate() + i)
        const list = lessons.filter(l => l.date === newD.toLocaleDateString('ru-RU'))
        return <Box key={i} className='dayOfWeek' borderWidth="1px" boxShadow="base" borderColor='blue' w='100%'>
            <Heading fontSize='1xl' p={4}>{getNameOfDay(i)}</Heading>
            <Divider />
            <TableContainer>
                <Table size='sm'>
                    <Thead>
                    ...
                    {RenderLessonRow({ list })}
                </Table>
            </TableContainer>
        </Box>
    }, [weekStart, lessons])

    return isLoading ? <Center><Spinner /></Center> : (
        <div>
